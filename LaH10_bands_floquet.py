...
for i in range(tot_num_of_k):
    print(i)
    H_k0_list = []
    for c in range(2*m_max+1):
        locals()['H_R' + str(c)] = np.zeros((lines), dtype = complex)
        locals()['H_k0' + str(c)] = np.zeros((num_wann , num_wann), dtype = complex)
        for j in range( 0 , lines ):
            d1= R[j][0]+r[n[j]-1][0]-r[m[j]-1][0]
            d2= R[j][1]+r[n[j]-1][1]-r[m[j]-1][1]
            d3= R[j][2]+r[n[j]-1][2]-r[m[j]-1][2]
            if (R[j].all == 0) and (m[j] == n[j]):
                locals()['H_R' + str(c)][j] += omega * (m_max-c) # Only apply +\Omega on E_site
            locals()['H_k0' + str(c)][m[j]-1][n[j]-1] += locals()['H_R' + str(c)][j] * e ** (1j*2*pi* (k1[i]*d1 +k2[i]*d2 +k3[i]*d3) )
        ...
        H_k0_list.append(locals()['H_k0' + str(c)])
    H_F0 = LA.block_diag(*(H_k0_list)) # Diagonal blocks creating a list of matrices for block_diag
    ...
    eigenvals = LA.eigvalsh(H_F).real
